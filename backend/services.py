...
def analyze_ai_content(description):
    """
    Estimate whether a report's writing style contains
    AI-like linguistic patterns.

    IMPORTANT:
    This is a transparent heuristic for prototype/demo use.
    It is NOT a definitive AI-content detector and should
    not be treated as proof that text was generated by AI.

    Returns:
        ai_content_likelihood: 0-100
        human_like_likelihood: 0-100
        detection_confidence: 0-100
        signals: explainable signals found in the text
        method: describes the analysis method
    """
    ...
    if not text:
        return {
            "ai_content_likelihood": 0,
            "human_like_likelihood": 100,
            "detection_confidence": 20,
            "signals": ["No text available for analysis"],
            "method": "Prototype heuristic text analysis"
        }
    ...
    if word_count < 5:
        return {
            "ai_content_likelihood": 10,
            "human_like_likelihood": 90,
            "detection_confidence": 25,
            "signals": [
                "Text is too short for reliable style analysis"
            ],
            "method": "Prototype heuristic text analysis"
        }
    ...
    # 1. Common generic/formal phrases frequently seen
    # in generated or highly templated text.
    ai_style_phrases = [
    ...
    if phrase_matches:
        ai_score += min(
            30,
            len(phrase_matches) * 8
        )
        signals.append(
            "Generic/formal phrasing detected"
        )

    # 2. Sentence-length regularity.
    # Highly uniform sentence lengths can be one
    # AI-like style signal, but this is only a weak signal.
    sentences = [
        sentence.strip()
    ...
    # 3. Repeated words / phrases.
    # Repetition is a weak supporting signal only.
    if word_count >= 12:
        word_frequency = {}
        ...
        if repeated_words:
            ai_score += min(
                15,
                len(repeated_words) * 5
            )
            signals.append(
                "Repeated terminology detected"
            )

    # 4. Highly structured punctuation/list style.
    # Keep this low-weight because genuine reports can
    # also be structured.
    structured_markers = len(
        re.findall(
            r"(?:^|\s)(?:[-*•]|\d+[.)])\s+",
            text,
            flags=re.MULTILINE
    ...
    if structured_markers >= 3:
        ai_score += 8
        signals.append(
            "Highly structured list-style formatting detected"
        )

    # 5. Very polished long-form structure.
    # This is intentionally weak because good human writing
    # can look exactly the same.
    if (
        word_count >= 80
        and len(sentences) >= 5
        and len(phrase_matches) >= 2
    ):
        ai_score += 12
        signals.append(
            "Long, highly structured narrative detected"
        )

    # 6. First-person / concrete observation signals.
    # These slightly reduce the AI-likelihood estimate.
    human_signals = []

    first_person_matches = re.findall(
        r"\b(?:i|we|my|our|me|us)\b",
        normalized
    )

    concrete_observation_patterns = [
        r"\b\d+(?:\.\d+)?\s*(?:°c|c|km/h|mm|cm|m)\b",
        r"\b(?:am|pm)\b",
        r"\b(?:today|yesterday|right now|currently)\b",
        r"\b(?:at|near|opposite|behind|outside)\b"
    ]

    if first_person_matches:
        human_signals.append(
            "First-person observation language detected"
        )

    concrete_count = sum(
        len(re.findall(pattern, normalized))
        for pattern in concrete_observation_patterns
    )

    if concrete_count > 0:
        human_signals.append(
            "Concrete observation details detected"
        )
    ...
    # Keep the estimate in a safe range.
    ai_score = max(
        0,
        min(
            round(ai_score),
    ...
    human_likelihood = 100 - ai_score

    # Confidence depends mainly on text length.
    if word_count < 10:
        confidence = 35
    elif word_count < 20:
        confidence = 50
    ...
    if len(signals) == 0:
        signals.append(
            "No strong AI-style signals detected"
        )

    return {
        "ai_content_likelihood": ai_score,
        "human_like_likelihood": human_likelihood,
...
def analyze_report(
    ...
    # AI CONTENT LIKELIHOOD
    # Supporting signal only. It does NOT directly change
    # the final trust score in this prototype.
    ai_content = analyze_ai_content(
        description
    )
    ...
    return {
        ...
        "trust_analysis": {
            ...
            "ai_content_likelihood":
                ai_content[
                    "ai_content_likelihood"
                ],

            "human_like_likelihood":
                ai_content[
                    "human_like_likelihood"
                ],
